...
<body>
        ...
        <div class="db-info">
            ...
            <div class="db-details">
                ...
                <div class="db-detail-item">
                    ...
                    <span class="db-value" style="color: var(--danger-color);">⚠️ VULNERABLE</span>
                </div>
                <div class="db-detail-item">
                    <span class="db-label">Protection Level:</span>
                    <span class="db-value" style="color: var(--danger-color);">❌ NO SANITIZATION</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('loginForm').addEventListener('submit', function(e) {
            ...
            let isInjection = injectionPatterns.some(pattern => 
                pattern.test(username) || pattern.test(password)
            );

            // Check for legitimate login first
            if (!isInjection && users[username] && users[username] === password) {
                ...
                if (
                    ...
                    // SQL injection detected but failed
                    showAlert('⚠️ SQL Injection detected but query failed. Try another payload!', 'error');
                }
            } else {
                // Invalid credentials (no injection)
